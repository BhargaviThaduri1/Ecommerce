...
document.body.style.backgroundColor = "#C8C8D8";
...
function navbar() {
    ...
    jewelryLink.textContent = "jewelery";
    jewelryLink.addEventListener("click", function () {
        productContainer.innerHTML = "";
        getDataProducts("jewelery");
    ...
    a3.innerText = "WISHLIST";
    a3.className = "nav-link btn text-white mx-1 ";
    ...
    var div = document.createElement('div');
    div.innerHTML = "\n<section class=\"home\">\n        <div class=\"container\" id=\"leftContainer\">\n            <h1>Live Your Life in</h1>\n            <h2>A New Style!</h2>\n            <p>\n                This say first impression is the last impression. Dont't Bless \n                Yourself with perfect clothing.\n            </p>\n            <button>Explore Now \u2192</button>\n        </div>\n        <div class=\"container\" id=\"rightContainer\">\n            <img src=\"img/home.png\" alt=\"\">\n        </div>\n    </section>\n  ";
    productContainer.append(div);
    var filter = document.createElement("div");
    var filterSection = document.createElement("div");
    filterSection.classList.add("filter-section");
    filter.appendChild(filterSection);
    // Create h4 element for price range
    var priceRangeTitle = document.createElement("h4");
    priceRangeTitle.textContent = "Price Range";
    filterSection.appendChild(priceRangeTitle);
    // Create input element for price range
    ...
    priceRange.value = "50";
    priceRange.step = "10";
    filterSection.appendChild(priceRange);
    // Create span element for minimum price range
    var minPrice = document.createElement("span");
    minPrice.classList.add("min-price");
    minPrice.textContent = "0";
    filterSection.appendChild(minPrice);
    // Create span element for maximum price range
    var maxPrice = document.createElement("span");
    maxPrice.classList.add("max-price");
    maxPrice.textContent = "200";
    filterSection.appendChild(maxPrice);
    // Create filter button
    var filterBtn = document.createElement("button");
    filterBtn.id = "filterBtn";
    filterBtn.className = "btn btn-success mx-3";
    filterBtn.textContent = "Filter";
    filterSection.appendChild(filterBtn);
    // Add event listener to price range input
    priceRange.addEventListener("input", function () {
        minPrice.textContent = "".concat(parseInt(priceRange.value) - 10);
        maxPrice.textContent = "".concat(priceRange.value);
    });
    filterBtn.addEventListener("click", function () {
        var filteredProducts = [];
        for (var i = 0; i < allProducts.length; i++) {
            var product = allProducts[i];
            var formattedPrice = parseInt(product.price.toString());
            if (formattedPrice >= parseInt(minPrice.textContent) && formattedPrice <= parseInt(maxPrice.textContent)) {
                filteredProducts.push(product);
            }
        }
        if (filteredProducts.length == 0) {
            alert("NO PRODUCTS FOUND IN THAT RANGE");
        }
        else {
            productContainer.innerHTML = "";
    ...
    for (var i = 0; i < allProducts.length; i++) {
        var product = allProducts[i];
        var formattedPrice = product.price.toString();
        if (parseInt(formattedPrice) >= parseInt(priceRange.value) - 10 && parseInt(formattedPrice) <= parseInt(priceRange.value)) {
            filteredProducts.push(product);
        }
    }
    productContainer.append(filter);
    displayProducts(filteredProducts);
}
var h3 = document.createElement('h3');
h3.textContent = "Sort by Price";
var gend = document.createElement('div');
var inc = document.createElement('button');
inc.className = "btn btn-secondary mx-2";
inc.textContent = "Inc";
...
dec.textContent = "Desc";
dec.className = "btn btn-secondary mx-2";
dec.addEventListener("click", function () {
    productContainer.innerHTML = "";
    getDataProducts("desc");
});
gend.appendChild(h3);
gend.appendChild(inc);
gend.appendChild(dec);
navbar();
productContainer.append(gend);
function sortIncProducts(sort) {
    getDataProducts("inc");
}
...
var ProductImpl = /** @class */ (function () {
    function ProductImpl(id, title, price, image, category, description, gender) {
        this.id = id;
        this.title = title;
        this.price = price;
        this.image = image;
...
function getDataProducts(category) {
    return __awaiter(this, void 0, void 0, function () {
        var response, productsData, products;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    allProducts = [];
                    if (!(category == 'desc')) return [3 /*break*/, 2];
                    return [4 /*yield*/, fetch('https://fakestoreapi.com/products?sort=desc')];
                case 1:
                    response = _a.sent();
                    return [3 /*break*/, 8];
                case 2:
                    if (!(category == 'inc')) return [3 /*break*/, 4];
                    return [4 /*yield*/, fetch('https://fakestoreapi.com/products?sort=asc')];
                case 3:
                    response = _a.sent();
                    return [3 /*break*/, 8];
                case 4:
                    if (!(category == "all")) return [3 /*break*/, 6];
                    return [4 /*yield*/, fetch("https://fakestoreapi.com/products")];
                case 5:
                    response = _a.sent();
                    return [3 /*break*/, 8];
                case 6: return [4 /*yield*/, fetch("https://fakestoreapi.com/products/category/".concat(category))];
                case 7:
                    response = _a.sent();
                    _a.label = 8;
                ...
                case 9:
                    ...
                    products.forEach(function (p) {
                        var newp = new ProductImpl(p.id, p.title, p.price, p.image, p.category, p.description, p.gender);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    tot_price = 0;
                    _loop_1 = function (i) {
                        var item, productData, title, image, price, quantity, cartItem, productImg, productInfo, productName, productPrice, quantityContainer, quantityLabel, decreaseBtn, quantityEl, increaseBtn, moveToWishlist;
                        return __generator(this, function (_b) {
                            switch (_b.label) {
                                ...
                                case 1:
                                    ...
                                    moveToWishlist = document.createElement("button");
                                    moveToWishlist.textContent = "Move to Wishlist";
                                    moveToWishlist.className = "btn btn-success my-4";
                                    moveToWishlist.addEventListener("click", function () {
                                        var wishData = JSON.parse(localStorage.getItem("wishData") || "[]");
                                        var productIndex = cartData.findIndex(function (item) { return item.prodId === productData.id; });
                                        if (productIndex == -1) {
                                            localStorage.setItem("wishData", JSON.stringify(productData.id));
                                        }
                                    });
                                    productInfo.appendChild(moveToWishlist);
...
function displayWishList() {
    return __awaiter(this, void 0, void 0, function () {
        var wishData, cartContainer, i, productData, title, image, price, cartItem, productImg, productInfo, productName, productPrice, quantityContainer;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    productContainer.innerHTML = "";
                    wishData = localStorage.getItem("wishData");
                    if (!(wishData && wishData.length)) return [3 /*break*/, 5];
                    cartContainer = document.createElement("div");
                    cartContainer.style.maxWidth = "600px";
                    cartContainer.className = "d-flex flex-wrap justify-content-center align-items-center  mx-auto";
                    cartContainer.classList.add("cart-container");
                    ...
                    _a.label = 1;
                case 1:
                    if (!(i < wishData.length)) return [3 /*break*/, 4];
                    return [4 /*yield*/, getProductDataById(parseInt(wishData[i]))];
                case 2:
                    productData = _a.sent();
                    title = productData.title, image = productData.image, price = productData.price;
                    cartItem = document.createElement("div");
